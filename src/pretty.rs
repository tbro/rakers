...
pub fn pretty_print(html: &str) -> String {
    ...
    while pos < bytes.len() {
        ...
        if bytes[pos] != b'<' {
            ...
            if !inner.is_empty() {
                if out.ends_with('\n') {
                    // Start of a new line: use indentation, no leading space.
                    push_indent(&mut out, indent);
                    out.push_str(inner);
                } else {
                    // Mid-line: preserve a leading space if the original had one.
                    if text.starts_with(|c: char| c.is_ascii_whitespace()) && !out.ends_with(' ') {
                        out.push(' ');
                    }
                    out.push_str(inner);
                }
                // Preserve trailing space for inline content (e.g. "Some " before <strong>).
